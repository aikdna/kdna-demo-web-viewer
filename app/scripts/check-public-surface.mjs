...
check(ciWorkflow.includes('1e77e3e0d486c330fe9f9262b514ef24c859d469'), 'browser CI must pin the exact Core fixture commit')
check(ciWorkflow.includes('fixtures/test_protected_entry.kdna'), 'browser CI must install the protected Core fixture')
check(ciWorkflow.includes('KDNA_PROTECTED_DEMO_ASSET'), 'browser CI must execute the protected fixture')
check(!fs.existsSync(path.join(repoRoot, '.github', 'workflows', 'release.yml')), 'frozen demo must not expose an automated release workflow')
